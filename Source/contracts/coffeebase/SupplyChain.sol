...
contract SupplyChain is Ownable, ConsumerRole, DistributorRole, FarmerRole, RetailerRole{
  ...
  /* remove because of duplicating with Ownable
  modifier onlyOwner() {
...
*/
  ...
  modifier checkValue(uint _upc) {
    ...
    //change to msg.sender as the caller maybe distributor or consumer
  ...
  modifier shipped(uint _upc) {
      require(items[_upc].itemState == State.Shipped, "Item has been ship");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Received
  modifier received(uint _upc) {
      require(items[_upc].itemState == State.Received,"Item has not received");
    _;
  }
  ...
  // and set 'sku' to 1
  // and set 'upc' to 1
  constructor() payable Ownable() FarmerRole() DistributorRole() ConsumerRole() RetailerRole(){
    //owner = msg.sender;
  ...
  function buyItem(uint _upc) forSale(upc) paidEnough(items[_upc].productPrice) checkValue(_upc) onlyDistributor public payable 
    // Call modifier to check if upc has passed previous supply chain stage
    ...
    payable(items[_upc].ownerID).transfer(items[_upc].productPrice);
